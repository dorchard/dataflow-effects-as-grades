...
-- Four-variable graded monad representing transfer functions as relations
...
-- Default meaning of if-then-else needs reinstating due to RebindableSyntax
...
g1 = do
 x <- getX
 -- Alternation form seems to work much better here
 -- but I think that's due to the unsolved nature of things here
 ifThenElseM (x >= 0) g2 g4
